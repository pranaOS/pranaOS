...
- If you are on macOS Big Sur, you will need to manually enable QEMU's acceleration before running pranaos, by creating a new file called `entitlements.xml` in the `Build/` folder, with the content below, and then run the command: `codesign -s - --entitlements entitlements.xml --force /usr/local/bin/qemu-system-x86_64`; otherwise the run command will fail.
...
For Windows, you will require Windows Subsystem for Linux 2 (WSL2). [Follow the WSL2 instructions here.](https://github.com/krishpranav/pranaos/blob/master/Documentation/NotesOnWSL.md)
