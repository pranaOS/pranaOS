/**
 * @file floatingpointstringconversions.cpp
 * @author Krisna Pranav
 * @brief Floating Point String Conversiosn
 * @version 6.0
 * @date 2025-03-26
 *
...
#include <mods/bigintbase.h>
#include <mods/charactertypes.h>
#include <mods/floatingpointstringconversions.h>
...
namespace Mods
{
    ...
    struct FloatingPointInfo
    {
        ...
        /**
         * @return constexpr SameSizeUnsigned 
         */
        ...
        /**
         * @tparam MaxPower 
         * @return constexpr Array<T, MaxPower + 1> 
         */
    ...
    /**
     * @tparam IsDoneCallback 
     * @tparam Has8CharsLeftCallback 
     * @param start 
     * @param is_done 
     * @param has_eight_chars_to_read 
    ...
    static BasicParseResult parse_numbers(char const* start, IsDoneCallback is_done, Has8CharsLeftCallback has_eight_chars_to_read)
    {
        ...
        [&]
        {
            ...
            ArmedScopeGuard reset_ptr{[&]
                                    { ptr = pointer_before_e; }};
            ++ptr;
    ...
    {
        Mods::Array<u128, highest_exponent - lowest_exponent + 1> values = {
    ...
    static FloatingPointBuilder binary_to_decimal(u64 mantissa, i64 exponent)
    {
        ...
        if(approximation_of_product_with_power_of_five.low() <= 1 && (real_mantissa & 0b11) == 0b01 && exponent >= FloatingPointRepr::min_exponent_round_to_even() && exponent <= FloatingPointRepr::max_exponent_round_to_even())
        {
    ...
    public:
        /**
         * @brief Construct a new Minimal Big Int object
         * 
         */
         ...
         * @param parse_result 
         * @param digits_parsed 
         * @param max_total_digits 
         * @return MinimalBigInt 
         */
        static MinimalBigInt from_decimal_floating_point(BasicParseResult const& parse_result, size_t& digits_parsed, size_t max_total_digits)
        {
            ...
            auto add_digits = [&](StringView digits, bool check_fraction_for_truncation = false)
            {
                ...
                while(parse_head != parse_end)
                {
                    while(max_word_counter - current_word_counter >= 8 && parse_end - parse_head >= 8 && max_total_digits - digits_parsed >= 8)
                    {
                    ...
                    while(current_word_counter < max_word_counter && parse_head != parse_end && digits_parsed < max_total_digits)
                    {
        ...
        void multiply_with_power_of_5(u32 exponent)
        {
            ...
            for(size_t i = 0; i < power_of_5.size(); ++i)
            {
                if(exponent >> i & 1)
                {
                    ...
                    {
                        auto copy = *this;
                        Ops::baseline_mul(copy.get_storage(), power_of_5[i], get_storage(m_used_length + power_of_5[i].size()), g_null_allocator);
    ...
    private:
         ...
         * @brief Get the storage object
         * 
         * @return UnsignedStorageReadonlySpan 
         */
        ...
        static constexpr Array<u64, 20> powers_of_ten_uint64 = {
            1UL, 10UL, 100UL, 1000UL, 10000UL, 100000UL, 1000000UL, 10000000UL, 100000000UL, 1000000000UL, 10000000000UL, 100000000000UL, 1000000000000UL, 10000000000000UL, 100000000000000UL, 1000000000000000UL, 10000000000000000UL, 100000000000000000UL, 1000000000000000000UL, 10000000000000000000UL};
    ...
    T parse_result_to_value(BasicParseResult& parse_result)
    {
        ...
        if(parse_result.mantissa <= u64(2) << FloatingPointRepr::mantissa_bits() && parse_result.exponent >= -FloatingPointRepr::max_exact_power_of_10() && parse_result.exponent <= FloatingPointRepr::max_exact_power_of_10() && !parse_result.more_than_19_digits_with_overflow)
        {
     ...
     * @tparam T 
     * @param parse_result 
     * @return constexpr FloatingPointParseResults<T> 
     */
    template <FloatingPoint T>
