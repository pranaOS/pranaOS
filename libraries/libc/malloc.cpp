...
#endif
    auto* ptr = pranaos_mmap(nullptr, size, PROT_READ | PROT_WRITE, flags, 0, 0, ChunkedBlock::block_size, name);
    VERIFY(ptr != nullptr);
    if (ptr == MAP_FAILED) {
        errno = ENOMEM;
        return nullptr;
...
/**
 * @param size 
 * @param caller_will_initialize_memory 
 * @return void* 
 */
