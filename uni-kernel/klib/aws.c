...
static buffer aws_create_can_req(heap h, sstring method, tuple req, buffer body)
{
    ...
    *(char*)buffer_ref(can_req, buffer_length(can_req) - 1) = '\n';
    ...
    if(mbedtls_sha256_ret(body ? buffer_ref(body, 0) : 0, body ? buffer_length(body) : 0, sha, 0) < 0)
        goto error;
                                        ...
                                        buffer datetime, buffer can_req)
{
    ...
    buffer_write(cred_scope, buffer_ref(datetime, 0), 8);
                                      ...
                                      buffer datetime, sstring secret, u8* key)
{
    ...
    if(!aws_hmac(ext_secret, sizeof(ext_secret), buffer_ref(datetime, 0), 8, k_date) ||
       ...
       !aws_hmac(k_region, sizeof(k_region), (const u8*)service.ptr, service.len, k_service) ||
...
void aws_req_set_date(tuple req, buffer b)
{
    ...
    struct tm tm;
    gmtime_r(&seconds, &tm);
    bprintf(b, "%d%02d%02dT%02d%02d%02dZ", 1900 + tm.tm_year, 1 + tm.tm_mon, tm.tm_mday, tm.tm_hour, tm.tm_min, tm.tm_sec);
                    ...
                    tuple req, buffer body, sstring access_key, sstring secret)
{
    ...
    if(!aws_hmac(signing_key, sizeof(signing_key), buffer_ref(string_to_sign, 0), buffer_length(string_to_sign), signature))
        goto free_string_to_sign;
    ...
    if(!buffer_write(auth, buffer_ref(datetime, 0), 8))
        goto free_auth;
