...
#define CASE_SYSREG(id)     \
    ...
    if((insn & 0xfff80000) == 0xd5380000)
    {
        ...
        u64* dest = &f[FRAME_X0] + (insn & 0x0000001f);
        *dest = val;
        frame_set_insn_ptr(f, u64_from_pointer(insn_ptr + 1));
        return true;
    }
    return false;
}
...
void start_secondary_cores(kernel_heaps kh)
{
    ...
    for(int i = 1; i < present_processors; i++)
        arm_hvc(PSCI_FN64_CPU_ON, mpid_from_cpuid(i), u64_from_pointer((void*)ap_start_nommu - kas_kern_offset + kernel_phys_offset), 0);
