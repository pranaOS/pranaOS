...
#define gic_debug(x, ...)                          \
    do                                             \
    {                                              \
        rprintf("%s: " x, func_ss, ##__VA_ARGS__); \
    } while(0)
#else
#define gic_debug(x, ...)
#endif

#define GIC_ICID(cpuid) (cpuid)
...
static void init_gicd_percpu(void)
{
    if(gic.v3_iface)
    {
        ...
        for(int i = GIC_PPI_INTS_START / GICD_INTS_PER_ICFG_REG;
            i < GIC_PPI_INTS_END / GICD_INTS_PER_ICFG_REG;
            i++)
            gicr_write_32(ICFGR(i), 0);
    }
    else
    {
...
#define gicc_write(reg, v)                     \
    do                                         \
    {                                          \
        if(gic.v3_iface)                       \
            write_psr_s(ICC_##reg##_EL1, (v)); \
        else                                   \
            mmio_write_32(GICC_##reg, v);      \
    } while(0)
#define gicc_set(reg, v)                       \
    do                                         \
    {                                          \
        gicc_write(reg, gicc_read(reg) | (v)); \
    } while(0)
#define gicc_clear(reg, v)                      \
    do                                          \
    {                                           \
        gicc_write(reg, gicc_read(reg) & ~(v)); \
    } while(0)
...
boolean dev_irq_enable(u32 dev_id, int vector, u32 target_cpu)
{
    ...
    if((vector >= gic_msi_vector_base) && gic.its_base)
    {
        ...
        if(!dev)
        {
            ...
            itt_size = MAX(itt_size, U64_FROM_BIT(8));
            ...
            gic_its_cmd(((u64)dev_id << 32) | ITS_CMD_MAPD, find_order(ite_num) - 1, ITS_MAPD_V | pa, 0);
        }
        u32 event_id = vector - gic_msi_vector_base;
        gic_its_cmd(((u64)dev_id << 32) | ITS_CMD_MAPTI, ((u64)vector << 32) | event_id, GIC_ICID(target_cpu), 0);
...
static void init_gicc(void)
{
    ...
    {
        mmio_write_32(GICC_CTLR, mmio_read_32(GICC_CTLR) | (GICC_CTLR_EnableGrp1 | GICC_CTLR_AckCtl));
    }
}
...
static void init_gits(kernel_heaps kh)
{
    ...
    for(int n = 0; n < 8; n++)
    {
        ...
        case GITS_TABLE_COLLECTIONS:
            ...
            gic_debug("allocating table type %d (entry size %d, page size %d)\n",
                      GITS_TABLE_TYPE(base),
                      GITS_TABLE_ENTRY_SIZE(base) + 1,
                      page_size);
    ...
    gits_write_32(CTLR, GITS_CTRL_ENABLED);
...
int init_gic(void)
{
    ...
    if(gic.v3_iface)
    {
        ...
        gic_msi_vector_num = MAX(gic_msi_vector_num, PAGESIZE);
        kernel_heaps kh = get_kernel_heaps();
        backed_heap backed = heap_linear_backed(kh);
        u64 pa;
