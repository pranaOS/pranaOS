...
TextEditorView::TextEditorView(UI::View* superview, const LG::Rect& frame, int ptmx)
...
TextEditorView::TextEditorView(UI::View* superview, UI::Window* window, const LG::Rect& frame, int ptmx)
...
void TextEditorView::recalc_dimensions(const LG::Rect& frame)
{
...
void TextEditorView::display(const LG::Rect& rect)
{
...
void TextEditorView::scroll_line()
{
    data_do_new_line();
    set_needs_display();
}

void TextEditorView::data_do_new_line()
{
...
WindowStatus TextEditorView::cursor_positions_do_new_line()
{
    m_col = 0;
    m_row++;
    if (m_row == m_max_rows) {
...
WindowStatus TextEditorView::cursor_position_move_right()
{
    m_col++;
    if (m_col == m_max_cols) {
...
WindowStatus TextEditorView::cursor_position_move_left()
{
    m_col--;
    if (m_col == 0 && m_row > 0) {
        m_row--;
...
void TextEditorView::increment_counter()
{
    will_move_cursor();
    m_col++;
    if (m_col == m_max_cols) {
...
void TextEditorView::decrement_counter()
{
    will_move_cursor();
    m_col--;
    if (m_col == 0) {
...
void TextEditorView::put_char(char c)
{
...
void TextEditorView::push_back_char(char c)
{
    if (c == '\n') {
        new_line();
        return;
...
void TextEditorView::put_text(const std::string& data)
{
...
void TextEditorView::send_input()
{
    write(ptmx(), m_input.c_str(), m_input.size() + 1);
    m_input.clear();
}

void TextEditorView::receive_keyup_event(UI::KeyUpEvent&)
{
}

void TextEditorView::receive_keydown_event(UI::KeyDownEvent& event)
{
