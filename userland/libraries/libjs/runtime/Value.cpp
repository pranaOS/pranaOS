...
static String double_to_string(double d)
{
    ...
    if (number_of_digits == 1) {
        ...
        builder.append(String::number(Base::abs(exponent - 1)));
        return builder.to_string();
    }
...
i32 Value::to_i32_slow_case(GlobalObject& global_object) const
{
    ...
    auto int32bit = remainder >= 0.0 ? remainder : remainder + 4294967296.0; 
...
i16 Value::to_i16(GlobalObject& global_object) const
{
    ...
    auto int16bit = remainder >= 0.0 ? remainder : remainder + 65536.0; 
    if (int16bit >= 32768.0)
        int16bit -= 65536.0;
...
i8 Value::to_i8(GlobalObject& global_object) const
{
    ...
    auto int8bit = remainder >= 0.0 ? remainder : remainder + 256.0;
    if (int8bit >= 128.0)
        int8bit -= 256.0;
