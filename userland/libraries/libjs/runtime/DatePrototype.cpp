...
void DatePrototype::initialize(GlobalObject& global_object)
{
    ...
    define_native_function(vm.names.setUTCDate, set_date, 1, attr); 
    ...
    define_native_function(vm.names.setUTCFullYear, set_full_year, 3, attr); 
    define_native_function(vm.names.getUTCHours, get_utc_hours, 0, attr);
    define_native_function(vm.names.setUTCHours, set_hours, 4, attr); 
    define_native_function(vm.names.getUTCMilliseconds, get_utc_milliseconds, 0, attr);
    define_native_function(vm.names.setUTCMilliseconds, set_milliseconds, 1, attr); 
    ...
    define_native_function(vm.names.setUTCMonth, set_month, 2, attr); 
    define_native_function(vm.names.getUTCSeconds, get_utc_seconds, 0, attr);
    define_native_function(vm.names.setUTCSeconds, set_seconds, 2, attr); 
...
JS_DEFINE_NATIVE_FUNCTION(DatePrototype::set_full_year)
{
    ...
    auto new_month = new_month_value.as_i32() + 1; 
