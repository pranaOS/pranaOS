...
class HostFunction {
public:
    explicit HostFunction(Base::Function<Result(Configuration&, Vector<Value>&)> function, FunctionType const& type)
...
private:
    Base::Function<Result(Configuration&, Vector<Value>&)> m_function;
    FunctionType m_type;
};
...
using InstantiationResult = Base::Result<NonnullOwnPtr<ModuleInstance>, InstantiationError>;
...
public:
    ...
    Base::Result<Vector<ExternValue>, LinkError> finish();
...
template<>
struct Base::Traits<Wasm::Linker::Name> : public Base::GenericTraits<Wasm::Linker::Name> {
