    ...
    auto operator()(Lhs lhs, Rhs rhs) const
    {
        if constexpr (IsFloatingPoint<Lhs>) {
            ...
            if (value.has_overflow())
                return Base::Result<Lhs, StringView>("Integer division overflow"sv);
            return Base::Result<Lhs, StringView>(value.value());
        }
    }
...
struct Modulo {
    ...
    auto operator()(Lhs lhs, Rhs rhs) const
    {
        ...
        if constexpr (IsSigned<Lhs>) {
            if (rhs == -1)
                return Base::Result<Lhs, StringView>(0); 
        }
        return Base::Result<Lhs, StringView>(lhs % rhs);
    }
...
struct Absolute {
    template<typename Lhs>
    auto operator()(Lhs lhs) const { return Base::abs(lhs); }
...
struct CheckedTruncate {
    template<typename Lhs>
    Base::Result<ResultT, StringView> operator()(Lhs lhs) const
    {
