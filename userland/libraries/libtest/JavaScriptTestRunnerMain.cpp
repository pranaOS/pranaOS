...
#else
        char* pranaos_source_dir = getenv("PRANAOS_SOURCE_DIR");
        if (!pranaos_source_dir) {
            warnln("No test root given, {} requires the PRANAOS_SOURCE_DIR environment variable to be set", g_program_name);
            return 1;
        }
        test_root = String::formatted("{}/{}", pranaos_source_dir, g_test_root_fragment);
        common_path = String::formatted("{}/userland/libraries/libjs/tests/test-common.js", pranaos_source_dir);
#endif
    }
