...
JOpenFlag stdio_parse_mode(const char *mode)
{
    JOpenFlag flags = 0;
...
{
    JOpenFlag flags = stdio_parse_mode(mode);
...
{
    JResult result = J_handle_close(file->handle);
    free(file);

    return result == JResult::SUCCESS ? 0 : -1;
}
...
size_t fread(void *ptr, size_t size, size_t count, FILE *file)
{
    ...
    {
        size_t read = 0;
        result = J_handle_read(file->handle, dst_ptr, size, &read);
        total += read;
        dst_ptr += read;
        if (result != JResult::SUCCESS)
        {
...
size_t fwrite(const void *ptr, size_t size, size_t count, FILE *file)
{
    ...
    {
        size_t written = 0;
        result = J_handle_write(file->handle, dst_ptr, size, &written);
        total += written;
        dst_ptr += written;
        if (result != JResult::SUCCESS)
        {
...
{
    JResult r = ERR_NOT_IMPLEMENTED;

    ssize64_t offset64 = offset;

    if (whence == SEEK_SET)
    {
        r = J_handle_seek(file->handle, &offset64, J_WHENCE_START, NULL);
    }
    else if (whence == SEEK_CUR)
    {
        r = J_handle_seek(file->handle, &offset64, J_WHENCE_CURRENT, NULL);
    }
    else if (whence == SEEK_END)
    {
        r = J_handle_seek(file->handle, &offset64, J_WHENCE_END, NULL);
    }

    return r == JResult::SUCCESS ? 0 : -1;
}

long ftell(FILE *stream)
{
    ssize64_t offset = 0;
    JResult r = J_handle_seek(stream->handle, nullptr, J_WHENCE_CURRENT, &offset);

    if (r != JResult::SUCCESS)
    {
        // TODO: check error
    }
...
{
    JResult r = J_filesystem_rename(oldpath, strlen(oldpath), newpath, strlen(newpath));
