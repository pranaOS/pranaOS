...
int clock_gettime(clockid_t clock_id, struct timespec* ts)
{
    if (Kernel::time_page_supports(clock_id)) {
        ...
        if (auto* kernel_time_page = get_kernel_time_page()) {
            ...
            do {
                update_iteration = Base::atomic_load(&kernel_time_page->update1, Base::memory_order_acquire);
                *ts = kernel_time_page->clocks[clock_id];
            } while (update_iteration != Base::atomic_load(&kernel_time_page->update2, Base::memory_order_acquire));
            return 0;
        }
    }
