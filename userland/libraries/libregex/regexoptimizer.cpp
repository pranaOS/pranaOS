/**
 * @file regexoptimizer.cpp
 * @author Krisna Pranav
 * @brief Regex Optimizer
 * @version 6.0
 * @date 2024-11-06
 * 
...
namespace regex 
{
     ...
     * @tparam Parser 
     * @param bytecode 
     * @return Regex<Parser>::BasicBlockList 
     */
    template<typename Parser>
    ...
    /**
     * @param bytecode 
     * @param repeated_block 
     * @param following_block 
     * @return AtomicRewritePreconditionResult 
     */
    static AtomicRewritePreconditionResult block_satisfies_atomic_rewrite_precondition(ByteCode const& bytecode, Block const& repeated_block, Block const& following_block)
    {
        ...
        dbgln_if(REGEX_DEBUG, "Found {} entries", repeated_values.size());
        dbgln_if(REGEX_DEBUG, "Found {} active groups", active_capture_groups.size());
    ...
    void Regex<Parser>::attempt_rewrite_loops_as_atomic_groups(BasicBlockList const& basic_blocks)
    {
        ...
        {
            DirectLoopWithoutHeader,               
            DirectLoopWithoutHeaderAndEmptyFollow, 
            DirectLoopWithHeader,                  
        }; // enum class AlternateForm 
        ...
        dbgln_if(REGEX_DEBUG, "Found {} blocks", candidate_blocks.size());
        if (candidate_blocks.is_empty()) {
            dbgln_if(REGEX_DEBUG, "Failed to find anything {}", pattern_value);
            return;
        }
    ...
    void Optimizer::append_alternation(ByteCode& target, Span<ByteCode> alternatives)
    {
        ...
        for (size_t i = alternatives.size(); i > 0; --i) {
            ...
            auto is_first = i == 1;
            auto instruction_size = entry.size() + (is_first ? 0 : 2); 
     ...
     * @param table 
     * @param pair 
     * @return LookupTableInsertionOutcome 
     */
    ...
    void Optimizer::append_character_class(ByteCode& target, Vector<CompareTypeAndValuePair>&& pairs)
    {
        ...
        target.empend(argument_count);   
        target.empend(arguments.size()); 
