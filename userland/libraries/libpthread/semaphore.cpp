...
{
    u32 value = Base::atomic_load(&sem->value, Base::memory_order_relaxed);
...
{
    u32 value = Base::atomic_fetch_add(&sem->value, 1u, Base::memory_order_release);

    if (!(value & POST_WAKES)) [[likely]]
        return 0;

    value = Base::atomic_fetch_and(&sem->value, ~POST_WAKES, Base::memory_order_relaxed);
...
int sem_trywait(sem_t* sem)
{
    ...
    bool exchanged = Base::atomic_compare_exchange_strong(&sem->value, value, desired, Base::memory_order_acquire);
    if (exchanged) [[likely]]
        return 0;
    else
        return EAGAIN;
...
int sem_timedwait(sem_t* sem, const struct timespec* abstime)
{
    ...
    while (true) {
        ...
        if (value == 0) {
            bool exchanged = Base::atomic_compare_exchange_strong(&sem->value, value, POST_WAKES, Base::memory_order_relaxed);
            if (!exchanged) [[unlikely]]
