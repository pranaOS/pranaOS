...
class Processor {
    ...
    u32 m_cpu;
    u32 m_in_irq;
    Atomic<u32, Base::MemoryOrder::memory_order_relaxed> m_in_critical;
...
public:
    ...
    {
        prev_flags = cpu_flags();
        u32 prev_crit = m_in_critical.exchange(0, Base::MemoryOrder::memory_order_acquire);
        if (!m_in_irq)
            check_invoke_scheduler();
    ...
    {
        m_in_critical.store(prev_crit, Base::MemoryOrder::memory_order_release);
