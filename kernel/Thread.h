...
public:
    ...
    u32 cpu() const { return m_cpu.load(Base::MemoryOrder::memory_order_consume); }
    void set_cpu(u32 cpu) { m_cpu.store(cpu, Base::MemoryOrder::memory_order_release); }
    ...
    [[nodiscard]] BlockResult block(const BlockTimeout& timeout, Args&&... args)
    {
        ...
        Atomic<bool, Base::MemoryOrder::memory_order_relaxed> timeout_unblocked(false);
    ...
    [[nodiscard]] bool has_unmasked_pending_signals() const { return m_have_any_unmasked_pending_signals.load(Base::memory_order_consume); }
    ...
    {
        if (m_is_active.load(Base::memory_order_acquire))
            return false;
    ...
    {
        VERIFY(refs_delta != 0);
        m_holding_locks.fetch_add(refs_delta, Base::MemoryOrder::memory_order_relaxed);
    ...
    {
        return m_holding_locks.load(Base::MemoryOrder::memory_order_relaxed);
    }
#endif
    ...
    {
        return m_nested_profiler_calls.fetch_add(1, Base::MemoryOrder::memory_order_acq_rel);
    }

    ALWAYS_INLINE u32 leave_profiler()
    {
        return m_nested_profiler_calls.fetch_sub(1, Base::MemoryOrder::memory_order_acquire);
    }
...
private:
    ...
    public:
        void thread_did_exit(void* exit_value)
        {
            ...
            m_exit_value.store(exit_value, Base::MemoryOrder::memory_order_release);
            do_unblock_joiner();
        }
        ...
        {
            VERIFY(m_thread_did_exit);
            return m_exit_value.load(Base::MemoryOrder::memory_order_acquire);
        }
    ...
    Atomic<bool, Base::MemoryOrder::memory_order_relaxed> m_is_active { false };
...
template<>
struct Base::Formatter<Kernel::Thread> : Base::Formatter<FormatString> {
