...
FsHandle::FsHandle(RefPtr<FsNode> node, JOpenFlag flags)
{
    _node = node;
    _flags = flags;
    _offset = 0;
...
{
    if (!has_flag(J_OPEN_READ) &&
        !has_flag(J_OPEN_SERVER) &&
        !has_flag(J_OPEN_CLIENT))
    {
...
JResult FsHandle::call(IOCall request, void *args)
{
    _node->acquire(scheduler_running_id());
    JResult result = _node->call(*this, request, args);
    _node->release(scheduler_running_id());

    return result;
}

JResult FsHandle::stat(JStat *stat)
{
    _node->acquire(scheduler_running_id());
...
ResultOr<RefPtr<FsHandle>> FsHandle::accept()
{
    ...
    auto connection_handle = make<FsHandle>(connection, J_OPEN_SERVER);
