...
void Plan9FS::ensure_thread()
{
    ...
    if (!m_thread_running.exchange(true, Base::MemoryOrder::memory_order_acq_rel)) {
        Process::create_kernel_process(m_thread, "Plan9FS", [&]() {
            thread_main();
            m_thread_running.store(false, Base::MemoryOrder::memory_order_release);
        });
    }
}
