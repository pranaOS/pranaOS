...
void SerialDevice::set_baud(Baud baud)
{
    ...
    m_base_addr.offset(3).out<u8>(m_base_addr.offset(3).in<u8>() | 0x80); 
    m_base_addr.out<u8>(((u8)(baud)) & 0xff);                             
    m_base_addr.offset(1).out<u8>(((u8)(baud)) >> 2);                     
    m_base_addr.offset(3).out<u8>(m_base_addr.offset(3).in<u8>() & 0x7f); 
}
