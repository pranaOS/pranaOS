...
#include <base/IntrusiveListRelaxedConst.h>
...
class Process
    ...
    BASE_MAKE_NONCOPYABLE(Process);
    BASE_MAKE_NONMOVABLE(Process);
...
public:
    ...
    bool is_dying() const { return m_state.load(Base::MemoryOrder::memory_order_acquire) != State::Running; }
    bool is_dead() const { return m_state.load(Base::MemoryOrder::memory_order_acquire) == State::Dead; }
    ...
    {
        return m_thread_count.load(Base::MemoryOrder::memory_order_relaxed);
    }
...
public:
    ...
    class ScopedDescriptionAllocation {
        BASE_MAKE_NONCOPYABLE(ScopedDescriptionAllocation);
...
private:
    ...
    Atomic<bool, Base::MemoryOrder::memory_order_relaxed> m_is_stopped { false };
...
template<>
struct Base::Formatter<Kernel::Process> : Base::Formatter<String> {
    void format(FormatBuilder& builder, const Kernel::Process& value)
    {
        return Base::Formatter<String>::format(builder, String::formatted("{}({})", value.name(), value.pid().value()));
    }
};
