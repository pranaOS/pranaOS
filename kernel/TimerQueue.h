...
private:
    ...
    void clear_cancelled() { return m_cancelled.store(false, Base::memory_order_release); }
    bool set_cancelled() { return m_cancelled.exchange(true, Base::memory_order_acq_rel); }

    bool is_in_use() { return m_in_use.load(Base::memory_order_acquire); };
    void set_in_use() { m_in_use.store(true, Base::memory_order_release); }
    void clear_in_use() { return m_in_use.store(false, Base::memory_order_release); }

    bool is_callback_finished() const { return m_callback_finished.load(Base::memory_order_acquire); }
    void clear_callback_finished() { m_callback_finished.store(false, Base::memory_order_release); }
    void set_callback_finished() { m_callback_finished.store(true, Base::memory_order_release); }
