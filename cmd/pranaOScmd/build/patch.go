...
const (
	pranaosModulePath = "github.com/pranaOS/pranaOS"
	pranaosImportFile = "import_pranaos.go"
	overlayFile       = "overlay.json"
)

var (
	pranaosImportTpl = template.Must(template.New("pranaos").Parse(`
	//+build pranaos

	package {{.name}}
	import _ "github.com/pranaOS/pranaOS"
	`))
)
...
func (b *Builder) pranaosImportFile() string {
	return filepath.Join(b.basedir, pranaosImportFile)
}
...
func (b *Builder) buildPrepare() error {
	...
	if !b.modHaspranaos() {
		log.Printf("pranaos not found in go.mod")
		err = b.editGoMod()
		if err != nil {
			return err
		}
	...
	err = b.writeImportFile(b.pranaosImportFile())
	if err != nil {
		return err
	}

	err = writeOverlayFile(b.overlayFile(), pranaosImportFile, b.pranaosImportFile())
	if err != nil {
		return err
	}
	return nil
...
func (b *Builder) modHaspranaos() bool {
	if b.currentModulePath() == pranaosModulePath {
		return true
	}
...
func (b *Builder) editGoMod() error {
	getPath := pranaosModulePath
	if b.cfg.pranaosVersion != "" {
		getPath = getPath + "@" + b.cfg.pranaosVersion
	}
...
func (b *Builder) writeImportFile(fname string) error {
	...
	err := pranaosImportTpl.Execute(&rawFile, map[string]interface{}{
		"name": pkgname,
	})
	if err != nil {
		return err
