...
Usage: $NAME COMMAND [TARGET] [ARGS...]
  ...
  Supported COMMANDs:
    ...
    build-toolchain: Deletes and re-builds the TARGET's toolchain
...
is_valid_target() {
    ...
    if [ "$TARGET" = "x86_64" ]; then
        CMAKE_ARGS+=("-DPRANAOS_ARCH=x86_64")
        return 0
    fi
...
cmd_with_target() {
    ...
    if [ ! -d "$PRANAOS_SOURCE_DIR" ]; then
        PRANAOS_SOURCE_DIR="$(get_top_dir)"
        export PRANAOS_SOURCE_DIR
    fi
    BUILD_DIR="$PRANAOS_SOURCE_DIR/Build/$TARGET"
    if [ "$TARGET" != "lagom" ]; then
        export PRANAOS_ARCH="$TARGET"
        TOOLCHAIN_DIR="$PRANAOS_SOURCE_DIR/Toolchain/Build/$TARGET"
    fi
}
...
build_toolchain() {
    ( cd "$PRANAOS_SOURCE_DIR/toolchain" && ARCH="$TARGET" ./BuildIt.sh )
}
...
run_gdb() {
    ...
    if [ "$TARGET" = "lagom" ]; then
        ...
        if [ -n "$KERNEL_CMD_LINE" ]; then
            export PRANAOS_KERNEL_CMDLINE="$KERNEL_CMD_LINE"
        fi
        sleep 1
...
if [[ "$CMD" =~ ^(build|install|image|run|gdb|test|rebuild|recreate|kaddr2line|addr2line|setup-and-run)$ ]]; then
    ...
    case "$CMD" in
        ...
        run)
            if [ "$TARGET" = "lagom" ]; then
                ...
                if [ -n "${CMD_ARGS[0]}" ]; then
                    export PRANAOS_KERNEL_CMDLINE="${CMD_ARGS[0]}"
                fi
                build_target run
            fi
            ;;
        ...
        test)
            ...
            if [ "$TARGET" = "lagom" ]; then
                ...
                build_target install
                build_target image
                export PRANAOS_KERNEL_CMDLINE="boot_mode=self-test"
                export PRANAOS_RUN="ci"
                build_target run
            fi
            ;;
        rebuild)
    ...
    if [ "$CMD" = "run" ]; then
        ...
        export PRANAOS_EXTRA_QEMU_ARGS="${PRANAOS_EXTRA_QEMU_ARGS} -d int -no-reboot -no-shutdown -S"
        set_tmux_title 'qemu'
        build_target run
    elif [ "$CMD" = "gdb" ]; then
        set_tmux_title 'gdb'
