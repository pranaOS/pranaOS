...
[ -d "$PRANAOS_SOURCE_DIR/sysroot" ] || die "$SPRANAOS_SOURCE_DIR/sysroot doesn't exist"
...
if rsync --chown 2>&1 | grep "missing argument" >/dev/null; then
    rsync -aH --chown=0:0 --inplace "$PRANAOS_SOURCE_DIR"/sysroot/ mnt/
    rsync -aH --chown=0:0 --inplace Root/ mnt/
else
    rsync -aH --inplace "$PRANAOS_SOURCE_DIR"/sysroot/ mnt/
...
if [ "$USE_CLANG_TOOLCHAIN" = "1" ]; then
    TOOLCHAIN_DIR="$PRANAOS_SOURCE_DIR"/Toolchain/Local/clang/"$PRANAOS_ARCH"
    mkdir -p mnt/usr/lib/clang/"$LLVM_VERSION"/lib/pranaos
    $CP "$TOOLCHAIN_DIR"/lib/clang/"$LLVM_VERSION"/lib/pranaos/* mnt/usr/lib/clang/"$LLVM_VERSION"/lib/pranaos
...
chmod -R g+rX,o+rX "$PRANAOS_SOURCE_DIR"/sysroot/* mnt/
...
cp -r "$PRANAOS_SOURCE_DIR"/userland/devtools/codex/languageservers/cpp/tests mnt/home/anon/cpp-tests/comprehension
cp -r "$PRANAOS_SOURCE_DIR"/userland/libraries/libcpp/tests mnt/home/anon/cpp-tests/parser
cp -r "$PRANAOS_SOURCE_DIR"/userland/libraries/libwasm/tests mnt/home/anon/wasm-tests
